...
data=load(fileNames(i)); % Charger directement le fichier
varNames = fieldnames(data); % Obtenir les noms des variables dans le fichier
    if numel(varNames) == 1
        RR = data.(varNames{1});
    end

RR = RR./1000; % RR is a vector containing RR intervals in seconds
RR= HRV.RRfilter(RR);
%RR = fillmissing(RR, 'linear');

Recording_time = 5 * 60; %(in seconds) 
Fs =  Recording_time/length(RR); % Sampling rate
Beg = round(Fs*90); % start of analysis
End = length(RR)- round(Fs*90); % end of analysis
